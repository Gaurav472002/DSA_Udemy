...
Node* AVL::Delete(Node *p, int key) {
    ...
    if (p->lchild == nullptr && p->rchild == nullptr){ // if the node is the root node
        if (p == root){
            root = nullptr;
        }
        delete p;
    ...
    if (BalanceFactor(p) == 2 && BalanceFactor(p->lchild) == 1) {  // L1 Rotation
        ...
        return LLRotation(p); // we can also do LR rotation here
    } else if (BalanceFactor(p) == -2 && BalanceFactor(p->rchild) == 0){  // R0 Rotation
        return RRRotation(p);  // we can also do RL rotation here
    }
