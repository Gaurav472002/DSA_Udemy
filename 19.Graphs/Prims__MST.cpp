...
void Prims_MST(int G[V][V], int n)
{
    ...
    int min = I;
    int near[V];
    int T[2][V - 2] = {0}; // 2d Array for storing the edges

    for (int i = 1; i < V; i++)
    {
        near[i] = I; //\inititalize near array with infinite values
        for (int j = 1; j < V; j++)
        {
            if (G[i][j] < min)
            {
    ...
    for (int i = 1; i < n - 1; i++)
    {
        ...
        for (int j = 1; j < V; j++) // update the near  array
        {
